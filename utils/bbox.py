    ...
    def decode_box(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            grid_x = np.repeat(
                np.expand_dims(
                    np.repeat(
                        np.expand_dims(np.linspace(0, input_width - 1, input_width), 0),
                        input_height,
                        axis=0,
                    ),
                    0,
            ...
            grid_y = np.repeat(
                np.expand_dims(
                    np.repeat(
                        np.expand_dims(
                            np.linspace(0, input_height - 1, input_height), 0
                        ),
                        input_width,
                        axis=0,
                    ).T,
            ...
            anchor_w = np.repeat(
                np.expand_dims(
                    np.repeat(
                        np.expand_dims(np.array(scaled_anchors)[:, 0], 0),
                        batch_size,
                        axis=0,
                    ),
                    -1,
            ...
            anchor_h = np.repeat(
                np.expand_dims(
                    np.repeat(
                        np.expand_dims(np.array(scaled_anchors)[:, 1], 0),
                        batch_size,
                        axis=0,
                    ),
                    -1,
            ...
            pred_boxes[..., 0] = x * 2.0 - 0.5 + grid_x
            pred_boxes[..., 1] = y * 2.0 - 0.5 + grid_y
            ...
            output = np.concatenate(
                [
                    np.reshape(pred_boxes, (batch_size, -1, 4)) / _scale,
                    np.reshape(conf, (batch_size, -1, 1)),
                    np.reshape(pred_cls, (batch_size, -1, self.num_classes)),
                ],
                -1,
            )
    ...
    def non_max_suppression(
        ...
        for i, image_pred in enumerate(prediction):
            ...
            class_pred = np.expand_dims(
                np.argmax(image_pred[:, 5 : 5 + num_classes], 1), -1
            )
            ...
            for c in unique_labels:
                ...
                output[i] = (
                    max_detections
                    if output[i] is None
                    else np.concatenate((output[i], max_detections))
                )
