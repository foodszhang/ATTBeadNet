...
class YOLOLoss(nn.Module):
    ...
    def bbox_iou(
        self, box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7
    ):
        box2 = box2.T
    ...
    def __call__(self, predictions, targets, imgs):
        ...
        bs, as_, gjs, gis, targets, anchors = self.build_targets(
            predictions, targets, imgs
        )
        # -------------------------------------------#
        #   计算获得对应特征层的高宽
        # -------------------------------------------#
        feature_map_sizes = [
            torch.tensor(prediction.shape, device=device)[[3, 2, 3, 2]].type_as(
                prediction
            )
        ...
        for i, prediction in enumerate(predictions):
            ...
            if n:
                ...
                t = torch.full_like(
                    prediction_pos[:, 5:], self.cn, device=device
                )  # targets
            ...
            # -------------------------------------------#
            obj_loss += (
                self.BCEobj(prediction[..., 4], tobj) * self.balance[i]
            )  # obj loss
    ...
    def build_targets(self, predictions, targets, imgs):
        ...
        for i in range(num_layer):
            matching_bs[i] = (
                torch.cat(matching_bs[i], dim=0)
                if len(matching_bs[i]) != 0
                else torch.Tensor(matching_bs[i])
            )
            matching_as[i] = (
                torch.cat(matching_as[i], dim=0)
                if len(matching_as[i]) != 0
                else torch.Tensor(matching_as[i])
            )
            matching_gjs[i] = (
                torch.cat(matching_gjs[i], dim=0)
                if len(matching_gjs[i]) != 0
                else torch.Tensor(matching_gjs[i])
            )
            matching_gis[i] = (
                torch.cat(matching_gis[i], dim=0)
                if len(matching_gis[i]) != 0
                else torch.Tensor(matching_gis[i])
            )
            matching_targets[i] = (
                torch.cat(matching_targets[i], dim=0)
                if len(matching_targets[i]) != 0
                else torch.Tensor(matching_targets[i])
            )
            matching_anchs[i] = (
                torch.cat(matching_anchs[i], dim=0)
                if len(matching_anchs[i]) != 0
                else torch.Tensor(matching_anchs[i])
            )
    ...
    def find_3_positive(self, predictions, targets):
        ...
        # ------------------------------------#
        ai = (
            torch.arange(num_anchor, device=targets.device)
            .float()
            .view(num_anchor, 1)
            .repeat(1, num_gt)
        )
        targets = torch.cat(
            (targets.repeat(num_anchor, 1, 1), ai[:, :, None]), 2
        ...
        for i in range(len(predictions)):
            ...
            # ----------------------------------------------------#
            anchors_i = torch.from_numpy(self.anchors[i] / self.stride[i]).type_as(
                predictions[i]
            )
            anchors_i, shape = (
                torch.from_numpy(self.anchors[i] / self.stride[i]).type_as(
                    predictions[i]
                ),
                predictions[i].shape,
            )
