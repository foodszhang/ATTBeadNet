...
class FullScaleSkipConnect(nn.Module):
    def __init__(self, 
        ...
        if am == CBAM:
            ...
            self.AM = nn.Sequential(nn.ReLU(), SimAM())
        else:
            self.AM = nn.Identity()
...
class U3PDecoder(nn.Module):
    def __init__(self, en_channels = [64, 128, 256, 512, 1024], skip_ch=64, dropout=0.3, fast_up=True, am=CBAM):
        super().__init__()
...
class UNet3Plus(nn.Module):
    ...
    def __init__(self, 
        ...
        self.decoder = U3PDecoder(self.encoder.channels[1:], skip_ch=skip_ch, dropout=dropout, fast_up=fast_up, am=am)
